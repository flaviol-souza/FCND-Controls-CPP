...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
	...
	differential_thrusts.y = momentCmd.y / l;
	differential_thrusts.z = -momentCmd.z / kappa;

	cmd.desiredThrustsN[0] = (collThrustCmd + differential_thrusts.x + differential_thrusts.y + differential_thrusts.z) / 4.f; // front left
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
	...
	float accelaration = (u_bar - CONST_GRAVITY) / R(2, 2);

	thrust = -mass * CONSTRAIN(accelaration, -maxAscentRate / dt, maxAscentRate / dt);
	/////////////////////////////// END STUDENT CODE ////////////////////////////  
...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
	...
	////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	V3F velCmdCap = velCmd;
	if (velCmd.mag() > maxSpeedXY) {
		velCmdCap = velCmd.norm() * maxSpeedXY;
	}
	V3F kpVel(kpVelXY, kpVelXY, 0.f);
	V3F kpPos(kpPosXY, kpPosXY, 0.f);
	accelCmd += kpVel * (velCmdCap - vel) + kpPos * (posCmd - pos);
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
	...
	float yawCmdPi = fmodf(yawCmd*term, 2 * F_PI)*term;
